diff --git a/tides/tidesapp/tidesapp.py b/tides/tidesapp/tidesapp.py
index d90f18e..d84a72b 100755
--- a/tides/tidesapp/tidesapp.py
+++ b/tides/tidesapp/tidesapp.py
@@ -1,454 +1,69 @@
 #!/bin/env python3
-"""
...
-                            date_time_combine)
+# Imports from the tidesapp project
...
+        Select the appropriate parse for the data in the user's input file
 
-               If no filename is passed in, a default list is loaded.
-
-        Returns: (nothing)
+        Args..
+        file - (str) A JSON-formatted file containing a list of dictionaries adhering to any of the supported modes.
 
-        Other:
+        Returns..
+        parser - A callable method for the selected mode's parsing
...
+            print("ERROR: input filename must not be 'None'")
+            raise ValueError
         else:
...
+                    return TideschartDotCom().retrieve_URLs
                 elif 'MUNIs' in data.keys():
-                    self.mode = Modes.MUNIs
-                    self.locations = data['MUNIs']
+                    return TideschartDotCom().retrieve_munis
                 else:
...
+        parser = self.choose_parser(file)
+        json_data = parser(file)
+        pass
...
-        driver.close()
+
+    # TODO: Recover search-for-locations code from old tidesapp.py in main branch (not newtides)
+
